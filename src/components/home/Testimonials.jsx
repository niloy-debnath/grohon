...
const Testimonials = () => {
  // Sample array representing your testimonial items matching the design structure
  const reviews = [
    {
      id: 1,
      ...
      name: "Rasel Ahamed",
      role: "CTO",
      avatar: "", // Add your local path or leave empty for initials placeholder
    },
    {
      id: 2,
      text: "A posture corrector works by providing support and gentle alignment to your shoulders, back, and spine, encouraging you to maintain proper posture throughout the day.",
  ...
  // Tracking index state for slider adjustments on smaller breakpoints if needed
  ...
  return (
    <section className="w-[91%] max-w-7xl mx-auto py-16 md:py-24 text-center font-urbanist  rounded-4xl my-6 overflow-hidden relative">
      ...
      {/* CAROUSEL FLOW GRID CONTAINER */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start px-4 md:px-8 max-w-6xl mx-auto relative mb-12">
        {reviews.map((review, index) => {
          ...
          return (
            <div
              key={review.id}
              className={`flex flex-col items-start text-left rounded-3xl p-8 transition-all duration-300 relative ${
                isCenter
                  ? "bg-white border border-gray-100 shadow-xl shadow-gray-200/60 scale-100 opacity-100 z-10"
                  : "scale-95 opacity-40 md:opacity-50 hidden md:flex"
              }`}
            >
              {/* LARGE DECORATIVE QUOTE ICON MARKS */}
              <div>
                <img src={quote} alt="" />
              </div>
              ...
              {/* HORIZONTAL DASHED SEPARATOR SEGMENT */}
              <div className="w-full border-t border-dashed border-gray-200/80 pt-6 mb-6" />

              {/* USER PROFILE META DATALAYER */}
              <div className="flex items-center gap-4">
                {/* Profile Circle Avatar container */}
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-white text-sm select-none ${
                    isCenter ? "bg-[#0D2C54]" : "bg-slate-300"
                  }`}
                >
                  {review.name
                    .split(" ")
                ...
                {/* Identity credentials text lines */}
      ...
      {/* CAROUSEL NAVIGATION AND PAGINATION ACTION LAYER */}
      <div className="flex items-center justify-center gap-6 mt-4 select-none">
        {/* Left Arrow Action Control */}
        <button
          onClick={() =>
            setActiveIndex((prev) =>
        ...
        {/* Center Progress Dot Matrix Indicators */}
          ...
          {reviews.map((_, dotIdx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                dotIdx === activeIndex ? "w-6 bg-[#0D2C54]" : "w-2 bg-gray-200"
              }`}
        ...
        {/* Right Arrow Action Control using brand emphasis color #FFB400 or lime-accent */}
        <button
          onClick={() =>
            setActiveIndex((prev) =>
